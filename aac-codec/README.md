...
Mediabunny *does* now ship their own `@mediabunny/aac-encoder` package (which wasn't available when I wrote this), but I'll continue using this one because I've heavily size-optimized it. A lot of things have been patched out to reduce the size of the WebAssembly module. In particular:
...
We also ship the WebAssembly directly instead of string-embedding it into the JS. The combined size optimizations bring us to 226KB vs. 594KB for `@mediabunny/aac-encoder`.
